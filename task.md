...
### **Role:** Claude API Integration, Sage Personality System, Core Logic

---

### **Task 1.1 — Build the Sage System Prompt Factory**

**Deliverable:** A reusable JavaScript/TypeScript module that generates system prompts for every Claude API call, with user context injected.
...
> 1. Takes userProfile object (from localStorage) with fields: name, motivation, subjects[], examDates[], streakCount, energyPeak
...
> 4. Returns the complete prompt as a string ready to be used as the 'system' parameter in Claude API calls
> Use Anthropic SDK: import Anthropic from '@anthropic-ai/sdk'. Write comments explaining each template variable."
...
**Deliverable:** A function that runs a multi-turn Claude conversation, then calls a second Claude API to parse the conversation into structured JSON userProfile.
...
> - For each user input (passed via onUserMessage), add to messages[], call Claude API with:
...
> - Claude returns structured UserProfile JSON
...
> - Use Anthropic SDK with streaming disabled (regular API, not streaming)
...
- Store raw chat history in localStorage under key: 'onboarding_chat_history'
```
...
**Deliverable:** A function that takes userProfile and calls Claude to generate a prioritized, constraint-aware study schedule for the next 2 weeks.
...
> 
> 3. **API Call:**
>    - Use Anthropic SDK, max_tokens = 4000
...
>    - If overlaps found, call Claude again with: 'The schedule has time conflicts. Fix overlaps and return corrected JSON.'
> 
> 5. **Return:**
>    - Return validated ScheduleBlock[] array
>    - Also auto-save to localStorage under key: 'schedule_blocks_' + today's date
...
- Each block type ratios (for Claude to understand):
...
- Claude should distribute ~60% of available hours to top-priority subject, 25% to second, 15% to third+
```
...
>    - Add all new blocks from Claude response
>    - Re-validate for time overlaps
>    - Sort by date + startTime
>    - Return merged array
> 
> 6. **Side Effects:**
>    - Update localStorage: 'schedule_blocks_' + today
...
**Deliverable:** Functions to verify study session proofs (photo, quiz, voice) using Claude vision.
...
> - Calls Claude API with vision: format = image/jpeg (or detect from data)
...
> - API calls use vision-enabled Claude model (claude-3-5-sonnet is safe choice)
...
> - Claude generates natural greeting, return the message string
> 
> **Part 2: Listen for Energy Response**
> - Wait for user input with energyLevel (1-5) and optional blocker text
> - If energyLevel == 1-2: call Claude with 'Reduce today's schedule by 50%. Replace with light revision and breaks. Keep only the most urgent topic.'
> - If energyLevel == 4-5: call Claude with 'Today is a high-energy day. Add one extra session for the hardest subject. Optimize for depth, not just coverage.'
> - If blocker mentioned: call Claude with 'The student mentioned: [blocker]. Adjust today's schedule to accommodate this. Show understanding first, then the adjusted plan.'
> 
> **Part 3: Return Adjusted Schedule**
> - Return: { checkInMessage, adjustedSchedule: ScheduleBlock[], energyLevel }
> - Auto-save adjustedSchedule to localStorage
...
**Additional Details:**
```
- Check-in should run once per calendar day (check localStorage for 'lastCheckInDate')
...
> - [ ] Extracted data persists to localStorage after onboarding completes
...
>    - If valid: update localStorage and call Claude to validate full schedule still meets exams
> 
...
>    - On mount: fetch schedule from localStorage
...
> 
> 10. **TypeScript:**
>     - Props: none (pulls data from localStorage and React Context or props passed from parent)
>     - State: selectedBlock, viewMode ('week' | 'day' | 'month')
>     - Use FullCalendar's EventClickArg type for click handlers
> 
> **Implementation Checklist:**
> - [ ] FullCalendar renders with correct events from localStorage
...
- Streak counter: fetch from localStorage key 'currentStreak'
...
>      * Brief feedback from Claude (from verifyPhotoProof or gradeQuizAnswers)
...
>    - localStorage: save session progress in case user navigates away
> 
> 8. **TypeScript:**
...
> - [ ] Session data logged to completionLog in localStorage"
...
- Pomodoro config saved to localStorage: 'pomodoroConfig'
...
- Store in localStorage under 'completionLog'
```
...
> 
> 8. **Data Flow:**
>    - Pull completionLog from localStorage
...
- Weekly report generated by Person 1's Claude call (implement as separate utility function)
...
>      → Claude calls helper functions from Person 1 to modify schedule
>      → Returns confirmation + updated view
>    - **Subject questions:** 'Explain the Carnot cycle'
>      → Claude provides concise explanation (2-3 sentences max, not a lecture)
>    - **Emotional support:** 'I don't think I can finish'
>      → Claude validates emotion, offers practical step
>    - **Data-driven insights:** 'Am I on track?'
>      → Claude checks completionLog + schedule, provides honest assessment with next actions
> 
> 4. **Suggested Prompts:**
...
>    → Sage immediately shifts to calm, empathetic tone (controlled by Claude system prompt variation)
...
**Additional Details:**
```
- Chat history storage: localStorage key 'chat_history' as JSON array
...
### **Role:** LocalStorage Architecture, State Management, Data Persistence

---

### **Task 3.1 — Data Persistence Layer (LocalStorage Schema)**

**Deliverable:** A well-structured localStorage schema and utility functions for reading/writing all app data.

**Exact Prompt to Code With:**
> "Create `src/lib/dataStore.ts` — a TypeScript module that manages all localStorage operations:
> 
...
>    - `calculateStorageUsed() → number` (returns approx size in bytes used by localStorage)
> 
> 6. **Error Handling:**
>    - Wrap all localStorage reads/writes in try-catch
>    - On write quota exceeded: throw error 'Local storage quota exceeded. Please clear some data.'
...
>    - Only active if localStorage key 'studyos_debug' = 'true'
> 
> **Implementation Checklist:**
> - [ ] All STORAGE_KEYS constants defined
> - [ ] Read functions return correct data from localStorage
...
**Additional Details:**
```
- All functions should be async (return Promise) even though localStorage is synchronous
...
- Debug mode accessible via: window.localStorage.setItem('studyos_debug', 'true') in browser console
...
>    - Loads userProfile, scheduleBlocks, completionLog from localStorage
>    - Provides context to all children
>    - On refresh (F5): restores state from localStorage automatically
> 
> 3. **Custom Hook:**
...
>    - `regenerateSchedule()`: calls Person 1's generateSchedule(userProfile), saves to localStorage, updates context
...
>    - `refreshAllData()`: re-fetches all data from localStorage, useful after importing data
> 
> 5. **Error Handling:**
...
>    - Load from localStorage: 100-500ms
>    - Set isLoading = false
...
> - [ ] Refresh works (tested with F5 and localStorage manual change)"
...
>    - Show storage used: 'You're using ~[X] MB of local storage'
> 
>    **F) About**
...
> 
> **Implementation Checklist:**
> - [ ] All settings persist to context + localStorage
...
- Storage calculation: add up all localStorage key sizes (rough estimate)
- Export defaults:
...
>    - Secondary button: 'I have an account' → checks localStorage for userProfile
...
> 
> 5. **Logic:**
>    - On mount: check if userProfile exists in localStorage
...
>    - For Person 1's Claude API calls that fail:
...
|--------|------|------------------|-----------|-----------|
| **Person 1** | AI & Backend Logic | System prompts, API integration, schedule generator, cascade rescheduler, proof verifier, check-in engine, weekly reports | Task 1.1 | Complete AI orchestration layer, all Claude calls work, schedule regenerates on missed sessions |
...
| **Person 3** | Data Management | LocalStorage schema, React Context, export/import, time utilities | Task 3.1 | Persistent data layer, global state management, all data survives refresh |
