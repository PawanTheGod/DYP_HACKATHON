...
**Primary AI Model:** Claude API (claude-sonnet-4-20250514) via `/v1/messages`  
...
|------------|----------------------|
| ✅ Working prototype in 12 hours | Expo/Next.js + Claude API — no heavy infra needed |
| ✅ No pre-built ML models | All intelligence via Claude API prompt engineering |
...
| API | Usage | Free Tier |
|-----|-------|-----------|
| **Anthropic Claude API** | Onboarding chat, schedule generation, companion messages, proof verification | Free tier / hackathon credits |
| **Web Speech API** (browser native) | Voice input during onboarding | Completely free, no API key |
| **LocalStorage / IndexedDB** | Persist user data client-side | Free, no backend needed |
| **Vercel** | Hosting & deployment | Free hobby tier |

### Optional Enhancements (still free)
| API | Usage |
|-----|-------|
| **Gemini API (Google)** | Fallback AI if Claude quota runs out |
...
2. `userProfile` → Sent to Claude API → Returns `scheduleJSON`
...
- After conversation ends, a second Claude call parses the chat into structured JSON:
```json
{
  "subjects": [
...
This score is passed to Claude API with the instruction: *"Allocate more daily blocks to higher-priority subjects proportionally."*
...
Claude returns a structured JSON array:
```json
[
  {
    "id": "block_001",
...
- Photo is sent to Claude API with vision: *"Does this image show evidence of studying [subject]? Return: { verified: true/false, confidence: 0-1, feedback: '...' }"*
- Verified sessions get a ✅ badge; unverified get ⚠️

#### Feature 3.2 — Quick Quiz Micro-Assessment
- After marking a session complete, Claude generates 3 quick MCQ questions on the topic studied
- User answers in-app
...
- Claude evaluates if the summary is plausible for the subject/topic
...
When a session is missed, Claude is called with:
```
...
- After manual reschedule, Claude re-evaluates if the overall plan is still on track for all deadlines
...
- These are generated by Claude with the system prompt containing the user's `motivation`
...
- Report is AI-generated via Claude, personalized to user's actual data
...
### 8.6 — Offline Mode
- Core schedule is cached in IndexedDB
...
- Claude returns a hierarchical topic list
...
> This is the system-level character definition that powers all of Sage's conversational behavior across the entire app. Paste this as the `system` parameter in every Claude API call.
...
- "I have an account" → login (localStorage check)
...
| Scenario | Handling |
|----------|----------|
| Claude API call fails | Show cached schedule; retry silently; notify user if >2 failures |
...
| Quiz answer submission | If Claude takes >5s, show loading state with "Grading your answers..." |
...
> "Write a JavaScript function `generateSchedule(userProfile)` that sends a POST request to the Anthropic Claude API at `https://api.anthropic.com/v1/messages`. The system prompt is the Sage character sheet. The user message is the stringified userProfile JSON with instruction to return a schedule as a JSON array. Parse the response and return the array."
...
**Cascade Rescheduler:**
> "Write a function `rescheduleAfterMiss(missedBlockId, currentSchedule, userProfile)` that calls Claude API with the missed block info and current schedule, asks Claude to redistribute the missed session across remaining slots without exceeding maxHours/day, and returns a new schedule array."

**Sage Check-In:**
> "Build a morning check-in modal that appears on first app open each day. It shows Sage's greeting (fetched from Claude based on today's schedule and user motivation), has a 1-5 energy rating selector, and a free-text 'any blockers?' input. On submit, call Claude to adjust today's schedule based on the energy rating."

---

*Document version: 1.0 | Project: PS-203 StudyOS | Build target: 12-hour hackathon prototype*  
*Free APIs used: Anthropic Claude, Web Speech API, FullCalendar, Recharts, Vercel, LocalStorage*
